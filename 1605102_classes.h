...
public:
    ...
    Point get_reflected_ray_direction(Ray &ray, Point normal) {
        Point out_dir = ray.dir - normal * 2.0 * dotProduct(ray.dir, normal);
...
public:
    ...
    Point getNormal(Point intersection) {
        Point normal = intersection - referencePoint;
...
public:
    ...
    double getIntersectionT(Ray &ray) {
        ...
        double t;
        bool flag = false;
        double denom = dotProduct(normal, ray.dir);

        if (denom < 0.0) {
            normal = normal * -1.0;
            denom = dotProduct(normal, ray.dir);
        }
        ...
        t = (dotProduct(normal, p1 - ray.start)) / denom;
        if (t >= 0)
...
void Object::fill_color(Ray &ray, double t, double current_color[3], int level) {
    ...
    for (int i = 0; i < lights.size(); i++) {
        double ambient = coEfficients[amb], lambert = 0.0, phong = 0.0;

        Point dir = (lights[i]->lightPos - intersectionPoint).normalize();
        ...
        if (!flag) {
            lambert = source_factor * coEfficients[dif] * dotProduct(L.dir, normal);
            phong = coEfficients[spec] * pow(dotProduct(R, V), shine);
        ...
        if (level < recursionLevel) {
            ...
            if (nearest != -1) {
                ...
                for (int k = 0; k < 3; k++) {
                    current_color[k] += reflected_color[k] * coEfficients[refl];
                }
            }
